...
const Exchange = memo(() => {
  ...
  return (
    <div className='flex' style={cardStyle}>
      <div style={{ width: '4rem', borderRight: '1px solid #ebebeb' }}>
        <div className='coinSet'>
          <i className='material-icons'>star_border</i>
        </div>
        {coinSets && coinSets.map((set) => <div className='coinSet'>{set.name}</div>)}
      </div>
      <div>
        <div style={{ padding: '0.5rem 0.75rem' }}>
          <div
            className='flex flex-v-center'
            style={{ border: '1px solid #ebebeb', padding: '0.25rem 0.5rem', width: '100%' }}
          >
            <i className='material-icons'>search</i>
            <input type='text' style={{ width: '100%' }} />
          </div>
        </div>
        <div className='flex flex-v-center px-1' style={{ fontSize: '0.75rem', width: '100%' }}>
          <div style={{ width: '1.5rem', height: '1.5rem', marginRight: '4px' }} />
          <div style={{ width: '1.5rem', height: '1.5rem', marginRight: '4px' }} />
          <div style={{ width: '6rem' }}>Coin</div>
          <div className='flex-grow-2'>Prices</div>
          <div style={{ textAlign: 'right' }}>Change</div>
        </div>
        <div className='box-content box-content-height'>
          {data && data.map((item) => <ExchangeRow key={item.id.toString()} item={item} />)}
        </div>
        <div className='box-button box-vertical-padding box-horizontal-padding'>
          <Link href='/market'>
            <span className='button button-purple button-medium button-block'>
              More
